...
1. Decision Trees - normal, ROSE, SMOTE
2. Boosted Decsision Trees - normal, ROSE, SMOTE
3. Penalized LDA
4. Penalized Logistic Regression (not used in the end)

For regression, we are using the following models:
1. xgboost linear regression
2. xgboost tweedie regression
3. LASSO Regression
4. Ridge Regression
5. Regression Splines

We ultimately used the following 2-step models:
1. Penalized LDA + LASSO Regression
2. Boosted Trees SMOTE + Regression Splines
3. Boosted Trees SMOTE + xgboost
...
```
The results of the penalized LDA+ LASSO/Ridge appears somewhat peculiar, with the exact same RMSE produced for training and test as well as LASSO/Ridge. Thus, we recommend further study regarding this approach.
...
Note that when applying the oversampling techniques like SMOTE, splitting into training and test set must occur before the SMOTE-ing. The model will then be trained on a SMOTE-d training set and later applied to a non oversampled test set.
```{r}
...
Ultimately, based on the results, we recommend the xgboost Tweedie Regression. We believe its relative simplicity will lead to more robust results vs the 2 step models which make more assumptions. 

Possible next steps: As mentioned, the LDA+ LASSO/Ridge model requires further study. There still could be improvements done with a weighted LASSO/Ridge regression. (The regresser does not make use of misclassification rates on the training data by default)

## Citations

Zhou, He & Yang, Yi & Qian, Wei. (2018). *Tweedie Gradient Boosting for Extremely Unbalanced Zero-inflated Data*.

Satwik Mishra (2017). *Handling Imbalanced Data: SMOTE vs. Random Undersampling*
